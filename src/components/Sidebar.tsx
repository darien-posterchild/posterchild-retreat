...
export default function Sidebar({ onTabChange, showTrialPromo = false }: SidebarProps) {
  ...
  useEffect(() => {
    if (activeParentId && !expandedParents[activeParentId]) {
      setExpandedParents((prev) => {
        ...
        try {
          localStorage.setItem('pc_sidebar_expanded_parents', JSON.stringify(next));
        } catch {
          // Ignore
        }
        return next;
  ...
  const handleToggleKey = (e: React.KeyboardEvent, targetCollapsed: boolean) => {
      ...
      <div className="pc-ref-sidebar__card">
        ...
        <div className="pc-ref-sidebar__header">
          {isCollapsed ? (
            ...
            <div className="pc-ref-sidebar__toggle-wrap group">
              ...
              >
                ...
                <div className="pc-ref-sidebar__expand-icon pc-ref-sidebar__expand-icon--reopen">
                  <PosterChildIcon name="layout-left" size={18} color="#404040" strokeWidth={1.8} />
                </div>
              </button>
            ...
            <>
              ...
              >
                <PosterChildIcon name="layout-left" size={18} color="#737373" strokeWidth={1.8} />
              </button>
            </>
          )}
        </div>
        ...
        >
          {PRODUCT_NAVIGATION.map((item) => {
            ...
            if (isCollapsed) {
              return (
                <div key={item.id} className="pc-ref-nav-item-wrap group">
                  <button
                    type="button"
                    onClick={() => handleNavigate(item.subPath, item.id)}
            ...
            return (
              <NavGroup key={item.id}>
                <NavItem
                  label={item.label}
                  icon={item.icon}
                  active={isParentDirectlyActive || (isParentActive && !isExpanded)}
                  childActive={isAnyChildActive}
                  expandable={hasChildren}
                  expanded={isExpanded}
                  onToggleExpand={(e) => toggleParentExpansion(item.id, e)}
                  onClick={() => {
                    if (hasChildren) {
                      if (isParentActive) {
                    ...
                    {item.children!.map((child) => {
                      ...
                      return (
                        <NavSubItem
                          ...
                          onClick={() => handleNavigate(child.subPath, item.id)}
                        />
                      );
                    })}
                  </div>
        ...
        <div className={`pc-ref-sidebar__footer ${isCollapsed ? 'is-collapsed' : ''}`}>
          {/* Secondary Nav */}
          <div className={`pc-ref-secondary-nav ${isCollapsed ? 'is-collapsed' : ''}`}>
            {isCollapsed ? (
              <>
                <div className="pc-ref-nav-item-wrap group">
                  <button
                    ...
                    aria-label="Support"
                  >
                    <PosterChildIcon name="life-buoy-01" size={20} color="#737373" strokeWidth={1.8} />
                  </button>
                  <div role="tooltip" className="pc-ref-collapsed-tooltip">
                    Support
                  </div>
                ...
                <div className="pc-ref-nav-item-wrap group">
                  <button
                    ...
                    aria-label="Settings"
                  >
                    <PosterChildIcon name="settings-01" size={20} color="#737373" strokeWidth={1.8} />
                  </button>
                  <div role="tooltip" className="pc-ref-collapsed-tooltip">
                    Settings
                  </div>
              ...
              <>
                <button type="button" className="pc-ref-secondary-item">
                  <span className="pc-ref-nav-item__icon-box">
                    <PosterChildIcon name="life-buoy-01" size={20} color="#A3A3A3" strokeWidth={1.8} />
                  </span>
                  <span className="pc-ref-nav-item__text">Support</span>
                  <span className="pc-ref-nav-item__trailing-icon">
                    <PosterChildIcon name="share-04" size={14} color="#A3A3A3" strokeWidth={1.8} />
                  </span>
                </button>
                <button type="button" className="pc-ref-secondary-item">
                  <span className="pc-ref-nav-item__icon-box">
                    <PosterChildIcon name="settings-01" size={20} color="#A3A3A3" strokeWidth={1.8} />
                  </span>
                  <span className="pc-ref-nav-item__text">Settings</span>
                </button>
              </>
            )}
          </div>

          {/* Free Trial Widget (Expanded Only, controlled by showTrialPromo feature flag) */}
          {!isCollapsed && showTrialPromo && (
            <div className="pc-ref-sidebar__trial-card">
              <div className="pc-ref-trial-header">
                <span className="pc-ref-trial-title">Free trial</span>
                <span className="pc-ref-trial-days">24 days left</span>
              </div>
              <div className="pc-ref-trial-progress">
                <div className="pc-ref-trial-progress-bar" style={{ width: '40%' }} />
              </div>
              <button type="button" className="pc-ref-trial-btn">
                Upgrade now
              </button>
            </div>
          )}
          ...
          {isCollapsed ? (
            ...
            <div className="pc-ref-nav-item-wrap group">
              ...
              >
                <img
                  src="/src/assets/screens/home/avatar-darien.png"
                  alt="Darien Menendez"
                  className="pc-ref-account-avatar-img"
                />
                <span className="pc-ref-account-status-dot" title="Online" />
              </div>
              <div role="tooltip" className="pc-ref-collapsed-tooltip">
                Darien Menendez
              </div>
            ...
            /* Expanded: Full Nav Account Card (228x64) */
            <div className="pc-ref-account-card">
              ...
              <div className="pc-ref-account-info">
                <span className="pc-ref-account-name">Darien Menendez</span>
                <span className="pc-ref-account-email">darien@poeterchild.ai</span>
              </div>
              <button type="button" className="pc-ref-account-action" title="Account options" aria-label="Account options">
